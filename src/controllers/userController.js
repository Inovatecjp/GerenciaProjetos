...
const creategerente = async (req, res) => {
  try {
    const user = await usersService.createUserProfileid(
      req.body,
      "073f3020-7f44-11ef-bc0f-9549c5af3b02"
    );
    res.status(200).json({ data: user });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
const createadmin = async (req, res) => {
  try {
    const user = await usersService.createUserProfileid(
      req.body,
      "2b530720-7f44-11ef-a189-a54a33318914"
    );
    res.status(200).json({ data: user });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
const createcolaborador = async (req, res) => {
  try {
    const user = await usersService.createUserProfileid(
      req.body,
      "2b552a00-7f44-11ef-a189-a54a33318914"
    );
    res.status(200).json({ data: user });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
const getUserWithoutPassword = async (req, res) => {
  try {
    const user = await usersService.getUserWithoutPassword(
      req.params.id || req.userInfo.id || req.session.user?.id
    );
    res.status(200).json({ data: user });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
const authenticate = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ token, message: "Login bem-sucedido", user, profile: profile1 });
  } catch (error) {
...
const myProjetos = async (req, res) => {
  try {
    const projetos = await projetoUsuarioService.getAssignmentByIdUsers(
      req.params.id || req.userInfo.id || req.session.user?.id
    );
...
const myprofile = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ perfis, grants: allGrants, message: "Login bem-sucedido" });
  } catch (error) {
    console.log(error);
...
const participantesPorProjeto = async (req, res) => {
  try {
    const projetos = await projetoUsuarioService.getAssignmentByIdProjetos(
      req.params.id
    );
